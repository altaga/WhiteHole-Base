...
/**
 * Main function to send a cross-chain message from Avalanche to Polygon.
 */
async function main() {
    try {
        ...
        // Set up the provider and wallet for Avalanche
        ...
        // Define the target chain and target address (the Polygon receiver contract)
        ...
        const recipientAddress = "0xd871d276c8dba0daa93828ce87f9f064bf4bfe38";
        const [iv, messFrom] = await encrypt(message, wallet.address);
        const [_, messTo] = await encrypt(message, recipientAddress, iv);
        ...
        // Send the message (ensure enough gas is sent in the transaction)
        const tx = await MessageSender.sendMessage(targetChain, targetAddress, gasLimit, recipientAddress, messFrom, messTo, iv, ethers.utils.parseUnits("0", 6), {
            value: txCost,
        });
